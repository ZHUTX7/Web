...
<body>
	...
	<div id="page-content" class="single-page">
		<div class="container">
			<div class="row">
				<div class="col-md-6">
					<div class="heading"><h2>修改商品信息</h2></div>
					<% request.setCharacterEncoding("utf-8");
					 int goods_id=Integer.parseInt(request.getParameter("goods_id"));
 						Goods gs=GoodsDB.getGoodsById(goods_id);
 						
					   if(gs!=null){ %>
  						<form method="post"	 action="modifyGoods"	>
  						<input type="hidden" name="goods_id" value=<%=gs.getGoods_id() %>>
  						<div class="form-group">
							<p>商品名称:<input type="text" name="goods_name" value="<%=gs.getGoods_name()%>" class="form-control" required>
						</div>
						<div class="form-group">
							<p>价格:<input type="text" name="goods_price" value="<%=gs.getGoods_price() %>" class="form-control" required>
						</div>
						
						<div class="form-group">
							<p>商品类型:<select   name="goods_type"  required>
						<option value="<%=gs.getGoods_type()%>" selected>请选择类型</option>
						...
						<div class="form-group">
							<p>备注:<input type="text" name="goods_note" value="<%=gs.getGoods_note()%>" class="form-control" required>
						</div>
						<div class="form-group">
							<p>图片:<input type="text" name="goods_img" value="<%=gs.getGoods_img()%>"class="form-control" required>
						</div>
						<div class="form-group">
