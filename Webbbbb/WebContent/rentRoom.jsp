...
<body>
	...
	<div id="page-content" class="single-page">
		<div class="container">
			<div class="row">
				<div class="col-md-6">
					<div class="heading"><h2>购买</h2></div>
					<%  request.setCharacterEncoding("utf-8");
					    Object us=session.getAttribute("user") ;
					    User user = (User)us;
					    int room_id=Integer.parseInt(request.getParameter("room_id"));
 						Room rm=RoomDB.getRoomById(room_id);
 						int sell_id=UserRoomDB.getUserId(room_id);//获取房东id
 						if(user!=null){%>
 						
 						<p>房屋地址：<%=rm.getRoom_adress() %>
 						<p>价格：<%=rm.getRoom_price() %>
  						
  						<form method="post"	 action="rentRoom"	>
  						<input type="hidden" name="room_id" value="<%=room_id %>" >
  						<input type="hidden" name="customer_id" value="<%=user.getUser_id() %>" >
						<input type="hidden" name="seller_id" value="<%=sell_id %>" >
						<input type="hidden" name="room_adress" value="<%=rm.getRoom_adress()%>" >
						<input type="hidden" name="room_price" value="<%=rm.getRoom_price()%>" >
						<div class="form-group"><p>请输租房起始日期:<input type="text" class="form-control" placeholder="年.月.日.  例如：2018.7.1:" name="begin_date"  required>
						</div>
						<div class="form-group"><p>请输租房结束日期:<input type="text" class="form-control" placeholder="年.月.日.  例如：2018.9.31" name="end_date"  required>
						</div>
						<div class="form-group">
							<input type="submit" class="btn"  value="签订合同并支付定金" >
						</div>
						</form>
						</div>
						<%}else{ %>
						
 							<a href="account.jsp">请先登陆账户~</a>
 						<%}
						%>
