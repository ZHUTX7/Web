...
<body>
	...
	<div id="page-content" class="single-page">
		<div class="container">
			...
			<div class="row">
				<div class="col-md-6">
					<div class="heading"><h2>出租房屋</h2></div>
					
  						<form method="post"	 action="addRoom"	>
  						<input type="hidden" name="user_id" value="<%=user.getUser_id() %>">
  						<div class="form-group">
							<p>地址:<input type="text" name="room_adress"  class="form-control" required>
						</div>
						<div class="form-group">
							<p>面积(平方米):<input type="text" name="room_area"  class="form-control" required>
						</div>
						<div class="form-group">
							<p>楼层:<input type="text" name="room_floor"  class="form-control" required>
						</div>
						
						<div class="form-group">
							<p>租金/月:<input type="text" name="room_price"  class="form-control" required>
						</div>
						<div class="form-group">
							<p>卧室数量:<select   name="room_type"  required>
						<option value="" selected>请选择卧室数量</option>
		...
		enctype="multipart/form-data">
			...
			Input文件：&nbsp<input type="file" name="file"
				...
				<div class="col-md-6">
					<div class="heading"><h2>已发布的出租信息：</h2></div>
						
 						<%List<UserRoom> list =UserRoomDB.getUserRoomByUser(user.getUser_id());
 						for (int i=0; i<list.size(); i++) {			
 							UserRoom ur=list.get(i);	
 							Room rm=RoomDB.getRoomById(ur.getRoom_id());
 							if(rm!=null){
 							%>
						<div class="form-group">
						<%=rm.getRoom_adress() %>  <%=rm.getRoom_price() %><a href="modifyRoom.jsp?room_id=<%=rm.getRoom_id()%>">|——修改——|</a><a href="delRoom?room_id=<%=rm.getRoom_id() %>">|——撤销出租——|</a><a href="setRoomID?room_id=<%=rm.getRoom_id()%>">上传图片</a>
						</div>
