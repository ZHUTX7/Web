...
<body>
	...
	<div id="page-content" class="single-page">
		<div class="container">
			...
			<div class="row">
				<div class="col-md-6">
					<div class="heading"><h2>申请发布广告</h2></div>
					
  						<form method="post"	 action="addAdervertisement"	>
  						<input type="hidden" name="user_id" value="<%=user.getUser_id() %>">
  						<div class="form-group">
							<p>广告超链接:<input type="text" name="ahref_url"  class="form-control" required>
						</div>
						
						<div class="form-group">
							<p>租金:<input type="text" name="ad_price"  class="form-control" required>
						</div>
						<div class="form-group">
	   ...
	   <form name="form2" action="uploadImg" method="post"
		enctype="multipart/form-data">
			...
			Input文件：&nbsp<input type="file" name="file"
				...
				<div class="col-md-6">
					<div class="heading"><h2>已申请的广告信息：</h2></div>
						<%List<Adervertisement> list =AdvertisementDB.getAdByUser(user.getUser_id());
 						for (int i=0; i<list.size(); i++) {			
 							Adervertisement ad=list.get(i);	
 						
 							%>
						<div class="form-group">
						<%=ad.getAhref_url() %>  <%=ad.getAd_price() %><a href="delAdApply?ad_id=<%=ad.getAd_id()%>">|——撤销申请——|</a>
						</div>
						<% }%>
					<div class="heading"><h2>已发布的广告信息：</h2></div>
						<%
						List<PublishAdvertisement> list2 =PublishAdDB.getAdByUser(user.getUser_id());
 						for (int i=0; i<list2.size(); i++) {			
 							PublishAdvertisement ad2=list2.get(i);	
 						
 							%>
						<div class="form-group">
						<%=ad2.getAhref_url() %>  <%=ad2.getAd_price() %><a href="modifyAd.jsp?ad_id=<%=ad2.getAd_id()%>">|——修改广告——|</a><a href="delAd?ad_id=<%=ad2.getAd_id()%>">|——撤销发布——|</a>
						</div>
						<% }%>
				</div>
			</div><!-- row -->
