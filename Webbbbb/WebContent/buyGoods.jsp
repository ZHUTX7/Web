					...
					<% request.setCharacterEncoding("utf-8");
					   Object us=session.getAttribute("user") ;
					    int goods_id=Integer.parseInt(request.getParameter("goods_id"));
 						...
 						int sell_id=UserGoodsDB.getUserId(goods_id) ;// userID卖家ID
 						if(user!=null){%>
 						
 						<p>商品名称：<%=gs.getGoods_name() %>
 						<p>价格：<%=gs.getGoods_price() %>
  						<form  method="post"	 action="buyGoods"	>
  						
  						<input type="hidden" name="customer_id" value="<%=user.getUser_id() %>" >
						<input type="hidden" name="seller_id" value="<%=sell_id %>" >
						<input type="hidden" name="goods_name" value="<%=gs.getGoods_name() %>" >
						<input type="hidden" name="goods_price" value="<%=gs.getGoods_price() %>" >
						<input type="hidden" name="goods_id" value="<%=goods_id%>" >
						<div class="form-group">
							<p>联系电话:<input type="text" name="customer_mobile"  class="form-control"  required>
						</div>
						
						<div class="form-group">
							<p>收货人姓名+收货地址:<input type="text" name="customer_adress"  class="form-control" required >
						</div>
						<div class="form-group">
							<input type="submit" class="btn"  value="提交并支付" >
						</div>
						</form>
						</div>
						<%} 
