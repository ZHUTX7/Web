...
<body>
   ...
   %>
    ...
    <a href="Backstage.jsp">返回主页</a>	
					...
					<div class="heading"><h2>收藏夹</h2></div>
					<% request.setCharacterEncoding("utf-8");
					Object us=session.getAttribute("user") ;
 						User user = (User)us;
 						if(user!=null){
 					List<Wish> list =WishDB.allWishByUserID(user.getUser_id());
 					    for (int i=(pages-1)*num; i<(pages-1)*num+num&&i<list.size(); i++) {			
 					    	Wish ms=list.get(i);
 					    	Goods gs=GoodsDB.getGoodsById(ms.getGoods_id());
 					%>
 						
  						<div class="form-group">
							<p><a href="Goods.jsp?goods_id=<%=gs.getGoods_id()%>">商品名称：<%=gs.getGoods_name() %> 价格：<%=gs.getGoods_price() %></a>  <a href="delWish?goods_id=<%=gs.getGoods_id()%>">删除</a>
						</div>
			...
			<a href="personalWish.jsp?pages=<%=q %> ">[<%=q%>]</a>	
<%} %>
